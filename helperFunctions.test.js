...
describe('Check All Operators For Missing Fields', () => {
  ...
  describe('Check Drivers License Numbers of Primary Fields', () => {

    
    let type = 'driversLicenseNumber';
    
    it('It should return 1 missing for driver\'s license number with "XXXXX"', () => {
      let { operators } = policies[0];
      result = getMissingFieldsAllOperators(operators, type);
      expect(result.length).toBe(1);
    });

    it('It should return 1 missing for driver\'s license number with "*****"', () => {
      let { operators } = policies[4];
      result = getMissingFieldsAllOperators(operators, type);
      expect(result.length).toBe(1);
    });
  });
